...
func TestRedisCache_Empty(t *testing.T) {
	...
	if inCache {
		t.Error("alpha found in cache but the cache should be empty")
	}
}
...
func TestRedisCache_EmptyByMatch(t *testing.T) {
	...
	err = testRedisCache.Set("alpha2", "foo2")
	if err != nil {
		t.Error(err)
	}
	err = testRedisCache.Set("beta", "foo3")
	if err != nil {
		t.Error(err)
	}
	...
	if !inCache {
		t.Error("beta did not found in cache, but it should be there")
	}
}
